...
const DUMMY_HISTORY = [
  {
    ...
    logs: [
      { type: 'motion', message: 'Motion detected around the device', time: '15:01 PM' },
  ...
  return (
    <View style={[styles.logItem, { backgroundColor }]}>
      ...
      <View style={styles.logTextContainer}>
        <Text style={[styles.logMessage, { color: messageColor }]}>{message}</Text>
...
export default function HistoryScreen() {
  ...
  const [filterType, setFilterType] = useState<'All' | 'motion' | 'on' | 'off'>('All');
  ...
  const getFilteredData = () => {
    return DUMMY_HISTORY.map(day => ({
      ...
      logs: day.logs.filter(log => {
        ...
        const matchesSearch = log.message.toLowerCase().includes(searchQuery.toLowerCase()) || day.date.toLowerCase().includes(searchQuery.toLowerCase());
      ...
      <ScrollView contentContainerStyle={styles.contentContainer}>
        {filteredHistory.length > 0 ? (
          ...
          <View style={styles.noHistoryContainer}>
            <Ionicons name="information-circle-outline" size={50} color={Colors.textLight} style={styles.noHistoryIcon} />
